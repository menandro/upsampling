%input image
% function [resultProp, resultOpt] = upsampling_instance(fname, semanticName, mainfolder)
    close all
    clear all

%     k = 63;
%     fname = '0000000278';%strcat('im',int2str(k));
%     semanticName = strcat('im',int2str(k));
...
%     flow = double(imread(strcat(mainfolder, 'output/depth/', fname, '.png')))/256.0;
    ...
    instance = imread(strcat(mainfolder, 'instance/sparse/', semanticName, '.png'));
    ...
    for i = M:-1:1
        count = count + 1;
        fprintf("vertical %d\n", count);
        imshow(ours, [0 40], 'ColorMap', jet);
        for j = 1:N        
            for k = 1:velo_ini
                if (velo_pos(k,1) >= j-windowRadius) && (velo_pos(k,1) < j+windowRadius) && (velo_pos(k,2) >= i-windowRadius) && (velo_pos(k,2) < i+windowRadius)
...
%                 fprintf("j=%d tmp=%d\n", j, tmp);
                ...
                if instance(py,px)~= instance(qy,qx)
                    
                elseif (velo_pos(k,1) >= j-10) && (velo_pos(k,1) < j+10) && (velo_pos(k,2) >= i-10) && (velo_pos(k,2) < i+10)
...
%             if i < M-windowRadius
%                 for k = velo_ini + ((M-i-windowRadius) * N) + 1 : velo_ini + ((M-i) * N)
%                     if (velo_pos(k,1) >= j-windowRadius) && (velo_pos(k,1) < j+windowRadius) && (velo_pos(k,2) >= i-windowRadius) && (velo_pos(k,2) < i+windowRadius)
%                         r = sqrt((j - velo_pos(k,1))*(j - velo_pos(k,1)) + (i - velo_pos(k,2))*(i - velo_pos(k,2)));
%                         if r0 > r
%                             r0 = r;
