%input image
function [resultProp, resultOpt] = upsampling_iv_pp(fname, semanticName, mainfolder)
    fprintf('No pixel proximity');
    %% faro
%     fname = 'im4';
%     semanticName = 'sem4';
        ...
        for j = 1:N        
            ...
            for k = 1:velo_ini
                if (velo_pos(k,1) >= j-10) && (velo_pos(k,1) < j+10) && (velo_pos(k,2) >= i-10) && (velo_pos(k,2) < i+10)
                    ...
                    sum_ = sum_ + (velo(k,1) * gr*(1.0/(gmax_+0.01))) * 1;
                    count_ = count_ + (gr*(1.0/(gmax_+0.01))) * 1;
                    if sem(i,j) > 0.27 && sem(i,j) < 0.28
                        sum_ = sum_ + (velo(k,1) * gr*(1.0/(gmax_+0.01))) * 10;
                        count_ = count_ + (gr*(1.0/(gmax_+0.01))) * 10;
                    end


                    if (velo_pos(k,1) >= 1) && (velo_pos(k,1) < N) && (velo_pos(k,2) >= 1) && (velo_pos(k,2) < M)
                        if sem(i,j) == sem(round(velo_pos(k,2)),round(velo_pos(k,1)))
                            sum_ = sum_ + (velo(k,1)*100*gr*(1.0/(gmax_+0.01))) * 1;
                            count_ = count_ + (100 * gr*(1.0/(gmax_+0.01))) * 1;
                            if sem(i,j) > 0.27 && sem(i,j) < 0.28
                                sum_ = sum_ + (velo(k,1)*10*gr*(1.0/(gmax_+0.01))) * 10;
                                count_ = count_ + (10 * gr*(1.0/(gmax_+0.01))) * 10;
                            end
                        end
                        if sem(i,j) == sem(round(velo_pos(k,2)),round(velo_pos(k,1)))

                            if sem(i,j) > 0.352 && sem(i,j) < 0.353
                                sum_ = sum_ + (velo(k,1)*10*gr*(1.0/(gmax_+0.01))) * 100;
                                count_ = count_ + (10 * gr*(1.0/(gmax_+0.01))) * 100;
                            end
                        end
                    end
                    gmax_ = 0;
            ...
            if i < M-10
                ...
                for k = velo_ini + ((M-i-10) * N) + 1:velo_ini + ((M-i) * N)
                    if (velo_pos(k,1) >= j-10) && (velo_pos(k,1) < j+10) && (velo_pos(k,2) >= i-10) && (velo_pos(k,2) < i+10)
                        ...
                        sum_ = sum_ + velo(k,1) * gr*(1.0/(gmax_+0.01));
                        count_ = count_ +  gr*(1.0/(gmax_+0.01));
                        if (velo_pos(k,1) >= 1) && (velo_pos(k,1) < N) && (velo_pos(k,2) >= 1) && (velo_pos(k,2) < M)
                            if sem(i,j) == sem(round(velo_pos(k,2)),round(velo_pos(k,1)))
                                sum_ = sum_ + velo(k,1)*100*gr*(1.0/(gmax_+0.01));
                                count_ = count_ + 100 * gr*(1.0/(gmax_+0.01));
                            end
                        end
                        gmax_ = 0;
                    end
