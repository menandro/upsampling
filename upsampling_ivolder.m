%input image
function [resultProp, resultOpt] = upsampling_ivolder(fname, semanticName, mainfolder)
    fprintf('Using new');
    %% faro
%     fname = 'im4';
%     semanticName = 'sem4';
        ...
        for j = 1:N        
            ...
            for k = 1:velo_ini
                if (velo_pos(k,1) >= j-10) && (velo_pos(k,1) < j+10) && (velo_pos(k,2) >= i-10) && (velo_pos(k,2) < i+10)
                    ...
                    T_pixelprox = 1.0 / (1.0 + sqrt((j - velo_pos(k,1))*(j - velo_pos(k,1)) + (i - velo_pos(k,2))*(i - velo_pos(k,2))));
                    T_depth = 1.0 / (1.0 + abs(velo(tmp,1) - velo(k,1)));
                    T_grad = (1.0/(gmax_+0.01));
